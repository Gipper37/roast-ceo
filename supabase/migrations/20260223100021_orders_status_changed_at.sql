-- Add status_changed_at to orders + update totals view
--
-- Problem: totals view was using orders.updated_at to detect when an order was
-- packed/delivered. Any subsequent edit to the order (delivery photo, notes, etc.)
-- bumps updated_at, causing the order to re-appear in packed_qty/delivered_qty
-- in a future week.
--
-- Solution: status_changed_at is set ONLY when order_status changes.
-- Edits to other fields never touch it.
...
-- updated_at is the best available approximation for when the status last changed.
...
DROP TRIGGER IF EXISTS trg_order_status_changed_at ON public.orders;

CREATE TRIGGER trg_order_status_changed_at
    BEFORE UPDATE ON public.orders
    FOR EACH ROW EXECUTE FUNCTION public.set_order_status_changed_at();

-- ─── 5. Update totals view: use status_changed_at instead of updated_at ───────
...
SELECT
    ...
    -- Packed Qty: orders whose status changed to Packed during THIS orders-week
    COALESCE((
        SELECT SUM(od.quantity)
        FROM public.order_details od
        JOIN public.orders o ON od.order_id = o.order_id
    ...
    -- Delivered Qty: orders whose status changed to Delivered during THIS orders-week
    COALESCE((
        SELECT SUM(od.quantity)
        FROM public.order_details od
        JOIN public.orders o ON od.order_id = o.order_id
