-- A card printed by mistake must be stoppable, or it circulates forever.
--
-- 20260908000024 gave bin_card voided_at/voided_by/void_reason and taught
-- resolve_bin_card to refuse a voided card — and then shipped no way to set
-- them. RLS grants SELECT only (deliberately: a card nobody can forge is the
-- point of letting close_pack_run trust one), so the columns were unreachable
-- and a card minted against the wrong batch would resolve cleanly for good.
-- Found by a test that tried to void one and updated zero rows.
--
-- Voiding is not the same act as printing. Anyone who logs a roast can print a
-- card; withdrawing one is saying a piece of paper already in the building is
-- wrong, which is a supervisor's call — so it rides on pack.void, the key that
-- already means "unwind a signed record", rather than pack.bin_card.
--
-- The card is NOT deleted. Somebody may already have bagged from it, and a
-- recall has to be able to see that the paper existed and was withdrawn.
...
begin
  ...
  select count(*) into v_used
    ...
    join public.bin_card_source bs on bs.roast_log_id = ps.roast_log_id
   where bs.bin_card_id = p_bin_card_id;
...
comment on function public.void_bin_card(text, text) is
  'Withdraw a printed bin card, with a reason. The row is kept — somebody may already have bagged from it, and a recall has to see that the paper existed and was pulled.';
