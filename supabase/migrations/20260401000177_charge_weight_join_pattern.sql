-- Migration 00177: Fix charge_weight to use standard ref/join pattern
--
-- roast_log.charge_weight stores the UUID key of charge_weight_options (the ref).
-- All math now resolves the value by JOINing to charge_weight_options,
-- exactly like every other ref table in the app.
-- The trigger no longer overwrites charge_weight — it leaves the UUID alone.
--
-- Step 1: Convert any existing numeric strings (legacy) to the correct UUID
-- Step 2: Update trg_stamp_roasted_weight — remove stamp-back
-- Step 3: Update all 4 functions
-- Step 4: Update all 6 views

-- Step 1 (data migration) was run directly via psql before this migration:
-- UPDATE roast_log SET charge_weight = cwo.id FROM charge_weight_options cwo
-- WHERE cwo.company_id = rl.company_id AND cwo.charge_weight = rl.charge_weight::numeric
-- AND rl.charge_weight ~ '^[0-9]+(\.[0-9]+)?$';
-- 3919 rows updated.

-- ── Step 2: trg_stamp_roasted_weight — join to resolve, don't overwrite ─────
...
BEGIN
    -- Resolve charge_weight UUID to numeric value via join
    SELECT cwo.charge_weight INTO v_charge_weight
    FROM public.charge_weight_options cwo
    WHERE cwo.id = NEW.charge_weight
    LIMIT 1;

    -- Fallback: if already a numeric string (legacy row), cast directly
    IF v_charge_weight IS NULL AND NEW.charge_weight ~ '^[0-9]+(\.[0-9]+)?$' THEN
        v_charge_weight := NEW.charge_weight::numeric;
    END IF;

    -- Do NOT overwrite charge_weight — leave the UUID (or legacy value) intact

    -- 3-tier retention factor lookup
...
BEGIN
    ...
    WHERE origin_id = p_origin_id AND facility_id = p_facility_id LIMIT 1;
    ...
    -- Direct roasts: join to charge_weight_options for numeric value
    SELECT COALESCE(SUM(cwo.charge_weight), 0) INTO v_roasted_direct_lbs
    FROM roast_log rl
    ...
    -- Blend roasts: join to charge_weight_options for numeric value
    SELECT COALESCE(SUM(cwo.charge_weight * rc.percentage), 0) INTO v_roasted_blend_lbs
    FROM roast_log rl
    JOIN roast_recipes rr ON rl.recipe_id = rr.recipe_id
...
BEGIN
    ...
    SELECT COALESCE(SUM(cwo.charge_weight), 0) INTO v_usage_direct
    FROM roast_log rl
    JOIN roast_recipes rr ON rl.recipe_id = rr.recipe_id
    JOIN charge_weight_options cwo ON cwo.id = rl.charge_weight
    WHERE rl.origin_id = p_origin_id
    ...
    SELECT COALESCE(SUM(cwo.charge_weight * rc.percentage), 0) INTO v_usage_blend
    FROM roast_log rl
    JOIN roast_recipes rr ON rl.recipe_id = rr.recipe_id
    JOIN recipe_components rc ON rl.recipe_id = rc.recipe_id
    JOIN charge_weight_options cwo ON cwo.id = rl.charge_weight
...
WITH roast_by_origin AS (
    SELECT
        rl.roast_date,
        rl.origin_id,
        cwo.charge_weight AS green_used_lbs,
...
SELECT
    ...
    COALESCE((SELECT sum(od.roasted_weight)
              FROM order_details od JOIN orders o ON od.order_id = o.order_id
              WHERE o.order_date >= c.order_week_start AND o.facility_id = c.facility_id
                AND o.order_status <> 'Canceled'), 0::double precision) / NULLIF(retention_rate::double precision, 0) AS total_ordered_green,
    ...
    COALESCE((SELECT sum(cwo.charge_weight)
              FROM roast_log rl
              JOIN charge_weight_options cwo ON cwo.id = rl.charge_weight
...
WITH direct_roasts AS (
    SELECT
        ...
        rl.origin_id,
        rl.facility_id,
        cwo.charge_weight AS green_lbs
    FROM roast_log rl
...
SELECT
    ...
    GREATEST(0::numeric, (
        COALESCE((SELECT ci.inventory_count_bags * ci.bag_size::numeric
                  FROM coffee_inventory ci
                  WHERE ci.origin_id = ds.origin_id AND ci.facility_id = ds.facility_id LIMIT 1), 0)
        + COALESCE((SELECT sum(p.purchased_lbs)
...
WITH facility_params AS (
    SELECT
        ...
        COALESCE((SELECT cp.value_number FROM company_parameters cp
                  WHERE cp.parameter_id = '761fd894' AND cp.facility_id = f.facility_id LIMIT 1), 25::numeric) AS charge_weight,
        COALESCE((SELECT cp.value_number FROM company_parameters cp
                  WHERE cp.parameter_id = '1de271df' AND cp.facility_id = f.facility_id LIMIT 1), 0.82) AS retention_rate
    FROM facilities f
), calc AS (
    SELECT
        fp.facility_id, fp.company_id, fp.timezone, fp.charge_weight, fp.retention_rate,
    ...
    SELECT
        of2.origin, of2.facility_id, of2.company_id,
        (COALESCE((SELECT sum(rsl.lbs_in_stock) FROM roast_stock_log rsl
                   WHERE rsl.origin_id = of2.origin AND rsl.facility_id = of2.facility_id
                     AND (rsl.created_at AT TIME ZONE c.timezone)::date >= c.roast_week_start), 0)
         + COALESCE((SELECT sum(rsl.lbs_in_stock * rc.percentage)
                     FROM roast_stock_log rsl JOIN recipe_components rc ON rsl.blend_id = rc.recipe_id
                     WHERE rc.coffee_item = of2.origin AND rsl.facility_id = of2.facility_id
                       AND (rsl.created_at AT TIME ZONE c.timezone)::date >= c.roast_week_start), 0)) AS in_stock_roasted,
        COALESCE((SELECT sum(od.quantity * p.weight_lbs * rc.percentage)
                  FROM order_details od
                  JOIN orders o ON od.order_id = o.order_id
                  JOIN products p ON od.product_id = p.product_id
                  JOIN recipe_components rc ON p.recipe_id = rc.recipe_id
                  WHERE rc.coffee_item = of2.origin AND o.order_status = 'Open' AND o.facility_id = of2.facility_id), 0) AS total_ordered,
        (COALESCE((SELECT sum(rl.roasted_weight) FROM roast_log rl
                   WHERE rl.origin_id = of2.origin AND rl."charged?" = true
                     AND rl.roast_date >= c.roast_week_start AND rl.facility_id = of2.facility_id), 0)
         + COALESCE((SELECT sum(rl.roasted_weight * rc.percentage)
                     FROM roast_log rl JOIN roast_recipes rr ON rl.recipe_id = rr.recipe_id JOIN recipe_components rc ON rl.recipe_id = rc.recipe_id
                     WHERE rr.roast_type = 'Pre-Blend' AND rc.coffee_item = of2.origin AND rl."charged?" = true
                       AND rl.roast_date >= c.roast_week_start AND rl.facility_id = of2.facility_id), 0)) AS total_roasted,
        c.retention_rate,
        COALESCE(
            (SELECT avg(cwo.charge_weight)
             FROM (SELECT rl.roast_date, cwo2.charge_weight
                   FROM roast_log rl
                   JOIN charge_weight_options cwo2 ON cwo2.id = rl.charge_weight
                   WHERE rl.origin_id = of2.origin AND rl.facility_id = of2.facility_id AND cwo2.charge_weight > 0
                   ORDER BY rl.roast_date DESC LIMIT 5) recent
             JOIN charge_weight_options cwo ON cwo.charge_weight = recent.charge_weight),
...
WITH facility_params AS (
    ...
    SELECT
        rf.recipe_id, rf.facility_id, rf.company_id,
        ...
        COALESCE(
            (SELECT avg(cwo.charge_weight)
             FROM (SELECT rl.roast_date, cwo2.charge_weight
                   FROM roast_log rl
                   JOIN charge_weight_options cwo2 ON cwo2.id = rl.charge_weight
                   WHERE rl.recipe_id = rf.recipe_id AND rl.facility_id = rf.facility_id AND cwo2.charge_weight > 0
                   ORDER BY rl.roast_date DESC LIMIT 5) recent
             JOIN charge_weight_options cwo ON cwo.charge_weight = recent.charge_weight),
            c.charge_weight, 25::numeric
        ) AS effective_charge_weight
    ...
    LEFT JOIN LATERAL (SELECT COALESCE(sum(rsl.lbs_in_stock), 0) AS in_stock_roasted
                       FROM roast_stock_log rsl
                       WHERE rsl.blend_id = rf.recipe_id AND rsl.facility_id = rf.facility_id
                         AND (rsl.created_at AT TIME ZONE c.timezone)::date >= c.roast_week_start) stock ON true
    LEFT JOIN LATERAL (SELECT sum(od.roasted_weight) AS total_ordered
                       ...
                       WHERE p.recipe_id = rf.recipe_id AND o.order_status = 'Open' AND o.facility_id = rf.facility_id) ordered ON true
    LEFT JOIN LATERAL (SELECT sum(rl.roasted_weight) AS total_roasted
                       FROM roast_log rl
                       WHERE rl.recipe_id = rf.recipe_id AND rl."charged?" = true
                         AND rl.roast_date >= c.roast_week_start AND rl.facility_id = rf.facility_id) roasted ON true
)
SELECT
    ...
    (GREATEST(0::double precision, (total_ordered - in_stock_roasted::double precision) - total_roasted::double precision)
        / NULLIF(retention_rate, 0)::double precision)
        / NULLIF(effective_charge_weight, 0)::double precision AS roasts_remaining
FROM per_recipe;
