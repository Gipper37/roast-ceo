-- Add batches_since_chaff to weekly_grand_total view.
-- Reads directly from the stored column on roast_log (same value for all
-- current charged rows per facility — MAX() picks it up cleanly).
