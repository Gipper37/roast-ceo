-- Tagged component counts reach recipe IS (owner-approved 2026-09-05).
--
-- The gap (cleanup items 3+5): counting leftover roasted single-origin
-- coffee never moved any recipe's In Stock, so the plan kept asking for
-- coffee that was already roasted. Two prior attempts (20260406, 20260420,
-- both reverted) credited an origin's count to EVERY recipe containing it —
-- phantom stock. The owner's design instead has the roaster TAG the count
-- with the recipe the coffee was roasted for: no allocation guesswork, the
-- person who roasted it does the attribution. Main use case: an unmarked
-- roast broke a recipe's rolled-forward estimate; one tagged count resets
-- the truth (counts are truth — standing principle).
--
-- What changes:
--   1. roast_stock_log gains a third row shape: stock_type='component'
--      carries BOTH blend_id (the recipe) and origin_id. Existing
--      blend/origin shapes untouched; new partial index mirrors theirs.
--   2. roast_detail_by_blend's COMPONENT LANE only: per (recipe, origin),
--      the latest component count within the 28-day window replaces that
--      component's share of the blend-derived estimate — IF the count is
--      newer than the blend anchor (latest count wins; a fresher blend
--      count beats a stale component count). In-current-week counts are
--      used verbatim; older ones roll forward exactly like the blend
--      anchor: + tagged roasts since (via roast_log_recipes, capped at
--      roast_week_start) − delivered since × percentage (capped at
--      orders_week_start). Clamped ≥ 0.
--   3. Untagged origin counts behave exactly as before: Coffee Group
--      display only. Pre-Blend / component-less recipes untouched.
--
-- No rows of type 'component' exist yet (UI ships separately), so this
-- migration is inert on apply: the view must be byte-identical for every
-- row until the first tagged count is logged.

begin;

insert into public.stock_types (stock_type_id, label, sort_order)
values ('component', 'Component', 3)
on conflict (stock_type_id) do nothing;

alter table public.roast_stock_log drop constraint roast_stock_log_ids_check;
alter table public.roast_stock_log add constraint roast_stock_log_ids_check check (
  (stock_type = 'blend'::text  and blend_id is not null and origin_id is null) or
  (stock_type = 'origin'::text and origin_id is not null and blend_id is null) or
  (stock_type = 'component'::text and blend_id is not null and origin_id is not null)
);

create index if not exists idx_roast_stock_log_component
  on public.roast_stock_log (blend_id, origin_id, facility_id, created_at desc)
  where stock_type = 'component'::text;
