-- Update process_staged_imports to collect session_ids and created_log_ids
-- so the revert/undo feature works after a successful import.
...
BEGIN
  ...
  UPDATE roastmaster_imports SET
    ...
    session_ids = CASE WHEN array_length(v_session_ids, 1) > 0 THEN v_session_ids ELSE NULL END,
    created_log_ids = CASE WHEN array_length(v_created_log_ids, 1) > 0 THEN v_created_log_ids ELSE NULL END
  WHERE import_id = p_import_id;
