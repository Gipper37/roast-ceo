-- Fix: process_staged_imports needs SECURITY DEFINER so it runs as the
-- function owner (postgres/superuser) and can bypass RLS on roast_sessions,
-- roast_temp_nodes, roast_events, and roast_log.
-- Without this, all INSERTs are silently denied because RLS is enabled on
-- those tables with no permissive policies for the authenticated role.
...
-- Grant execute to authenticated users (anon should not call this directly)
REVOKE ALL ON FUNCTION process_staged_imports(text, text, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION process_staged_imports(text, text, text, boolean) TO authenticated;
