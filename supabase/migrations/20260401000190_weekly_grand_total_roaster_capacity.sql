-- Update capacity_pct to use sum of active roaster unit capacities
-- instead of the flat facility-level parameter.
-- Falls back to facility parameter if no roaster units exist.
...
WITH facility_config AS (
    SELECT f.facility_id,
           ...
           -- Facility-level capacity fallback
           COALESCE((SELECT cp.value_number
                       FROM company_parameters cp
                    ...
                    (SELECT sp.amount
                      ...
                      LIMIT 1), 35) AS facility_capacity_hrs
      FROM facilities f
), calc AS (
    SELECT fc.facility_id,
           ...
           -- Total capacity = sum of active roaster unit overrides,
           -- falling back to facility default for units without an override,
           -- falling back to facility parameter if no roaster units at all.
           COALESCE(
               NULLIF((SELECT SUM(COALESCE(ru.capacity_hrs_per_week, fc.facility_capacity_hrs))
                         FROM public.roaster_units ru
                        WHERE ru.facility_id = fc.facility_id AND ru.is_active = true), 0),
               fc.facility_capacity_hrs
...
SELECT facility_id AS open_order_total_id,
       ...
       ROUND(
           ...
           * COALESCE((
           ...
           / NULLIF(c.total_capacity_hrs, 0) * 100
       , 1) AS capacity_pct
  FROM calc c;
