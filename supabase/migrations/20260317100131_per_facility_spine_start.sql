-- Migration 00131: Start each facility's date spine from its own first order date
--
-- Bug: order_graphs_week used a global MIN(order_date) for the spine start,
-- then CROSS JOINed all facilities. Facilities that started later got rows of
-- zeros going back to the earliest facility's first order.
--
-- Fix: filter the spine so each facility only appears from the week of its
-- first non-canceled order onward.
...
facility_starts AS (
    SELECT facility_id, date_trunc('week', MIN(order_date)::timestamp)::date AS first_week
    FROM public.orders
...
SELECT
    ...
    ROUND(AVG(avg_weekly_gross_profit)    OVER w3, 2) AS gross_profit_3yr_avg,
