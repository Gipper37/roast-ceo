-- The production date reaches the rest of the plan math — the owner's call
-- (2026-08-30), closing the two gaps he named:
--
-- 1. TRAILING AVERAGES bucket by COALESCE(production_date, order_date). The
--    6-week average exists to predict weekly roast demand, and the week that
--    matters is when the coffee was needed, not when the paperwork was typed.
--    An early-entered future order used to spike its ENTRY week's average —
--    and since buffer_target = avg_weekly_lbs x backstock_buffer_pct, that
--    spike inflated the backstock buffer for six weeks. Now such an order
--    joins the average only once its production week is actually history.
--    Fixed in BOTH averages: roast_detail_by_blend (recipe + component) and
--    totals.recent_avg_week.
--
-- 2. THE PACK QUEUE DEFERS future-production lines. You can't pack what the
--    plan hasn't roasted: an Open line whose order carries a production date
--    beyond this orders-week leaves left_to_pack/open_backlog until its week
--    arrives. Display-only — the order detail can still pack early from
--    shelf stock; the queue just stops nagging about work that isn't due.
--    Week membership (total / packed / delivered) also keys on the coalesced
--    date, so every surface tells one story.
--
-- pack_totals_by_prep gets the same two treatments so the grind sub-lines
-- keep reconciling against the group totals beside them.
...
create or replace view public.totals as
 WITH facility_params AS (
         ...
         SELECT p.product_id,
            ...
            f.company_id
           FROM products p
             JOIN facilities f ON p.company_id = f.company_id AND (p.facility_id IS NULL OR p.facility_id = f.facility_id)
        )
 SELECT (pf.product_id || '-'::text) || pf.facility_id AS totals_id,
    ...
    COALESCE(( SELECT sum(od.quantity) AS sum
           FROM order_details od
             JOIN orders o ON od.order_id = o.order_id
          WHERE od.product_id = pf.product_id AND COALESCE(o.production_date, o.order_date) >= c.orders_week_start AND COALESCE(o.production_date, o.order_date) < (c.orders_week_start + '7 days'::interval) AND o.facility_id = pf.facility_id AND o.order_status <> 'Canceled'::text), 0::numeric) AS total,
    COALESCE(( SELECT sum(od.quantity) AS sum
           FROM order_details od
             JOIN orders o ON od.order_id = o.order_id
          WHERE od.product_id = pf.product_id AND od.item_status = 'Open'::text AND (o.production_date IS NULL OR o.production_date < (c.orders_week_start + '7 days'::interval)) AND o.facility_id = pf.facility_id AND o.order_status <> 'Canceled'::text), 0::numeric) AS left_to_pack,
    COALESCE(( SELECT sum(od.quantity) AS sum
           FROM order_details od
             JOIN orders o ON od.order_id = o.order_id
          WHERE od.product_id = pf.product_id AND od.item_status = 'Open'::text AND COALESCE(o.production_date, o.order_date) < c.orders_week_start AND (o.production_date IS NULL OR o.production_date < (c.orders_week_start + '7 days'::interval)) AND o.facility_id = pf.facility_id AND o.order_status <> 'Canceled'::text), 0::numeric) AS open_backlog,
    COALESCE(( SELECT sum(od.quantity) AS sum
           FROM order_details od
             JOIN orders o ON od.order_id = o.order_id
          WHERE od.product_id = pf.product_id AND od.item_status = 'Packed'::text AND COALESCE(o.production_date, o.order_date) >= c.orders_week_start AND o.facility_id = pf.facility_id AND o.order_status <> 'Canceled'::text), 0::numeric) AS packed_qty,
    COALESCE(( SELECT sum(od.quantity) AS sum
           FROM order_details od
             JOIN orders o ON od.order_id = o.order_id
          WHERE od.product_id = pf.product_id AND o.order_status = 'Delivered'::text AND COALESCE(o.production_date, o.order_date) >= c.orders_week_start AND o.facility_id = pf.facility_id), 0::numeric) AS delivered_qty,
    COALESCE(( SELECT avg(sub.weekly_sum) AS avg
           FROM ( SELECT sum(od2.quantity) AS weekly_sum
                   FROM order_details od2
                     JOIN orders o2 ON od2.order_id = o2.order_id
                  WHERE od2.product_id = pf.product_id AND COALESCE(o2.production_date, o2.order_date) >= (c.orders_week_start - '42 days'::interval) AND COALESCE(o2.production_date, o2.order_date) < c.orders_week_start AND o2.facility_id = pf.facility_id AND o2.order_status <> 'Canceled'::text
                  GROUP BY (date_trunc('week'::text, COALESCE(o2.production_date, o2.order_date)::timestamp with time zone))) sub), 0::numeric) AS recent_avg_week
...
select o.facility_id, o.company_id, od.product_id,
       od.coffee_prep,
       sum(od.quantity) filter (where od.item_status = 'Open'
                                  and (o.production_date is null
                                       or o.production_date < (c.orders_week_start + interval '7 days'))) as left_to_pack,
       sum(od.quantity) filter (where od.item_status = 'Packed'
                                  and coalesce(o.production_date, o.order_date) >= c.orders_week_start) as packed_qty
  from order_details od
