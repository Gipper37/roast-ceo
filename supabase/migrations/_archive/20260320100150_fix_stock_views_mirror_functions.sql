-- Migration 00150: Fix stock views to mirror calculate_current_stock_lbs() / calculate_current_stock_consumables() exactly
--
-- Fixes:
-- 1. Delete bogus consumable_inventory_history record (2000-01-01, count=0) driving spine to Jan 2000
-- 2. Coffee stock: add "charged?" = TRUE filter (uncharged test roasts were inflating outflows)
-- 3. Coffee stock: add roast_type split (Pre-Blend vs direct) matching the function exactly
-- 4. Both views: add GREATEST(0, ...) floor — same as the live functions

DELETE FROM consumable_inventory_history
WHERE inventory_date = '2000-01-01'::date AND inventory_count = 0;

-- ============================================================
-- monthly_coffee_stock_by_origin
-- ============================================================
...
WITH
direct_roasts AS (
    -- Single-origin roasts (non-Pre-Blend), charged only
...
blend_roasts AS (
    -- Pre-Blend roasts allocated to component origins, charged only
...
-- ============================================================
-- monthly_consumable_stock_by_item
-- ============================================================
...
WITH
all_usage AS (
    SELECT o.order_date AS usage_date, pc.consumable_id, od.facility_id,
...
all_purchases AS (
    SELECT sr.date_received::date AS received_date, p.consumable_inventory_item AS consumable_id,
           p.facility_id, p.amount::numeric AS purchased_units
    FROM consumable_inventory_purchased p
    JOIN shipment_received sr ON sr.shipment_id = p.shipment_id
    WHERE sr.date_received IS NOT NULL
),
consumables AS (
    SELECT DISTINCT consumable_inventory_id AS consumable_id, facility_id, company_id,
...
date_spine AS (
    SELECT c.consumable_id, c.facility_id, c.company_id, c.item_name,
           gs.month_start::date AS month_start
    FROM consumables c
