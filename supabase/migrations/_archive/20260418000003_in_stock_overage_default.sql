-- 00218: roast_detail_by_blend.in_stock_roasted falls back to last-week overage
--
-- Today the view's in_stock_roasted = SUM(roast_stock_log entries this roast
-- week). When the user hasn't manually entered any stock, that's 0 — even
-- if last week they over-roasted relative to what was ordered (those extra
-- lbs are still sitting on the rack as available stock).
--
-- New behavior: when this-week roast_stock_log is 0, fall back to
--   max(0, last_week_roasted - last_week_ordered)
-- per recipe. Manual stock entries (roast_stock_log) still win when
-- present so the user can override.
...
WITH facility_params AS (
  ...
  SELECT rf.recipe_id,
    ...
    -- in_stock_roasted: prefer this-week manual stock log entries; if 0,
    -- fall back to last-week overage (lbs roasted last week minus lbs
    -- ordered last week, capped at 0).
    COALESCE(
  ...
  LEFT JOIN LATERAL (
    -- Last-week roasted (between previous roast week start and this one)
  ...
  LEFT JOIN LATERAL (
    -- Last-week ordered (orders that landed in the previous orders week)
