-- Fix weekly_grand_total: empty string timezone crashes AT TIME ZONE operator
--
-- facilities.time_zone can be '' (empty string), not just NULL.
-- COALESCE only catches NULL; NULLIF converts '' to NULL first so COALESCE
-- can substitute the fallback 'UTC'.
--
-- Change: COALESCE(f.time_zone, 'UTC')
--      → COALESCE(NULLIF(f.time_zone, ''), 'UTC')
