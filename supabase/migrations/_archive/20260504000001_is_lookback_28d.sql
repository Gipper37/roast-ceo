-- 20260504000001_is_lookback_28d.sql
-- Shorten the in-stock running-balance anchor lookback from 56 days
-- (8 weeks) to 28 days (4 weeks).
--
-- WHY
--   The original 56-day window (introduced in 20260423000007) lets the
--   IS calc accumulate net over-roast across two months when no manual
--   stock count is logged. Real roasted-coffee shelf life is 2-4 weeks
--   before customers reject it as stale, so anything older than 4 weeks
--   is effectively gone from saleable inventory whether it was sold or
--   discarded. 28 days aligns the model with industry shelf-life
--   reality and brings IS into a believable range without requiring a
--   manual stock-log discipline that doesn't yet exist.
--
--   This does NOT fix the root cause of stale stock counts — it just
--   bounds how stale the inferred IS can get. A future migration can
--   add an order-delivery-based consumption signal so we can reason
--   about real consumption instead of assuming sales == orders.
--
-- WHAT
--   Two literal changes inside roast_detail_by_blend:
--     1. recipe_anchor LATERAL: anchor lookup window
--     2. recipe_is running balance: anchor_date fallback
--   Other view logic unchanged. Re-emits the full view to keep the
--   definition source-of-truth in one place.
           ...
           ELSE GREATEST(0::numeric,
             ...
             - COALESCE((
                 ...
                 WHERE p.recipe_id = ra.recipe_id
                   ...
                   AND o.order_date >= COALESCE(ra.anchor_date, ra.orders_week_start - INTERVAL '28 days')
