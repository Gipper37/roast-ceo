-- Migration 00148: Fix stock views to not require a manual count
-- When no manual count exists for an origin/item, default anchor to (1900-01-01, 0)
-- so stock is computed as 0 + all purchases − all usage from the beginning.
-- NULL only appears if there is genuinely no data at all.

-- ============================================================
-- monthly_coffee_stock_by_origin (fixed)
-- ============================================================
...
origins AS (
    SELECT DISTINCT
        origin_id, facility_id, company_id, bag_size, origin AS origin_name
...
date_spine AS (
    SELECT
        o.origin_id, o.facility_id, o.company_id, o.bag_size, o.origin_name,
        gs.month_start::date AS month_start
    FROM origins o
    JOIN origin_first_event fe
        ON fe.origin_id = o.origin_id AND fe.facility_id = o.facility_id
...
-- most recent manual count before month-end; defaults to (1900-01-01, 0) if none
LEFT JOIN LATERAL (
...
-- purchases received after anchor (or from all time if no anchor) through month-end
LEFT JOIN LATERAL (
...
-- green roasted after anchor (or from all time if no anchor) through month-end
LEFT JOIN LATERAL (
...
-- ============================================================
-- monthly_consumable_stock_by_item (fixed)
-- ============================================================
...
date_spine AS (
    ...
    JOIN consumable_first_event fe
        ON fe.consumable_id = c.consumable_id AND fe.facility_id = c.facility_id
    JOIN LATERAL generate_series(
        date_trunc('month', fe.first_event::timestamp),
        date_trunc('month', NOW()),
        INTERVAL '1 month'
...
-- most recent manual count before month-end; defaults to (NULL, 0) if none exists
LEFT JOIN LATERAL (
...
-- units purchased after anchor (or from all time if no anchor) through month-end
LEFT JOIN LATERAL (
...
-- units used after anchor (or from all time if no anchor) through month-end
LEFT JOIN LATERAL (
