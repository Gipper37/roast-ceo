-- ============================================================================
-- Fix roast_detail_by_blend.total_ordered: scope to this-week's orders, all
-- non-cancelled statuses (matches avg_weekly_lbs convention in the same view).
--
-- Before:
--   total_ordered = SUM(roasted_weight) WHERE order_status = 'Open'
--     • No date filter (counts all-time Open orders ever for the recipe)
--     • Excludes Packed + Delivered (so a roastery whose orders all flow
--       through Packed before delivery sees total_ordered = 0)
--
-- Symptom: Social Hour US's Roast page showed ~all batches as BS and the
-- Roast Plan still asked for more BS roasting, because total_ordered
-- collapsed to 0 despite 53 real this-period orders (28 Packed + 25
-- Delivered, 0 Open). The frontend BS walk reads total_ordered as
-- "this week's demand" — when demand=0 the excess walk marks every
-- trailing batch as BS.
--
-- After:
--   total_ordered = SUM(roasted_weight) WHERE
--     order_status <> 'Canceled'
--     AND order_date >= orders_week_start
--     AND order_date <  orders_week_start + 7d
--   (Same shape avg_weekly_lbs already uses elsewhere in this view.)
--
-- Touches both LATERAL subqueries — recipe_level + component_final.
-- The two sites were identical bugs.
-- ============================================================================
           ...
           FROM recipe_facility rf
             ...
             LEFT JOIN LATERAL ( SELECT sum(od.roasted_weight) AS total_ordered
                   FROM order_details od
                     JOIN orders o ON od.order_id = o.order_id
                     JOIN products p ON od.product_id = p.product_id
                  WHERE p.recipe_id = rf.recipe_id AND o.facility_id = rf.facility_id AND o.order_status <> 'Canceled'::text AND o.order_date >= c.orders_week_start AND o.order_date < (c.orders_week_start + interval '7 days')) ordered ON true
